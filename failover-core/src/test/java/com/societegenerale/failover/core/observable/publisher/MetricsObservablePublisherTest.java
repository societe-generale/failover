...
class MetricsObservablePublisherTest {
    ...
    private MetricsObservablePublisher metricsObservablePublisher;

    @BeforeEach
    void setUp() {
        var logger = (ch.qos.logback.classic.Logger) LoggerFactory.getLogger(Logger.ROOT_LOGGER_NAME);
        logger.addAppender(appender);
        metricsObservablePublisher = new MetricsObservablePublisher();
    }

    @Test
    @DisplayName("should publish metrics")
    void shouldPublishMetrics() {
        metricsObservablePublisher.publish(METRICS);

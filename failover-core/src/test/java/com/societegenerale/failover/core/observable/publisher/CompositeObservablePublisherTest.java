...
class CompositeObservablePublisherTest {

    private static final Instant NOW = Instant.now();

    @Mock
    private ObservablePublisher publisher1;

    @Mock
    private ObservablePublisher publisher2;
    ...
    private CompositeObservablePublisher compositeObservablePublisher;

    @BeforeEach
    void setUp() {
        metrics = Metrics.of("failover");
        compositeObservablePublisher = new CompositeObservablePublisher(Arrays.asList(publisher1, publisher2), failoverClock);
    }
    ...
    void shouldPublishMetricsWithAllPublishers() {
        ...
        compositeObservablePublisher.publish(metrics);

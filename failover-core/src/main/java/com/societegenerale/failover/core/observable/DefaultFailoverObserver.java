...
/**
 * Default {@link FailoverObserver} that scans for all {@code @Failover}-annotated methods
 * and publishes a startup summary metric for each one.
 *
 * @author Anand Manissery
 */
public class DefaultFailoverObserver implements FailoverObserver {
     ...
     *
     * @param observablePublisher         publisher that receives each failover metric
     ...
     */
    public DefaultFailoverObserver(ObservablePublisher observablePublisher, FailoverScanner failoverScanner, FailoverClock clock, ManifestInfoExtractor manifestInfoExtractor, FailoverExpiryExtractor failoverExpiryExtractor, Map<String, String> additionalInfo) {
    ...
    public void observe() {
        ...
        failoverScanner.findAllFailover().forEach(failover->  {
            Metrics metrics = Metrics.of("failover-report-%s".formatted(failover.name()))
                    ...
                    .collect("domain", failover.domain())
                    .collect("key-generator", failover.keyGenerator())
                    ...
                    .collect("expiry-policy", failover.expiryPolicy())
                    .collect("payloadSplitter",  failover.payloadSplitter())
                    .collect("recover-all", String.valueOf(failover.recoverAll()));

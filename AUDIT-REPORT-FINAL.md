...
| #    | Issue                                                                                 | Category              | Criticality | Status                                                                                                                                                                                      |
|------|---------------------------------------------------------------------------------------|-----------------------|:-----------:|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| I-01 | No evidenced automated dependency/CVE scanning gate in CI                             | Security/Supply-chain |  🟠 MEDIUM  | ◑ PARTIAL — `.github/dependabot.yml` present (alerts); no dependency-check/Trivy build gate                                                                                                 |
| I-02 | Package-granular allowlist invites broad prefix over-grants                           | Security              |  🟠 MEDIUM  | ⬜ TODO — no exact-class preference / package-prefix WARN                                                                                                                                    |
| I-03 | Scatter/gather partial-recovery caller-facing contract                                | Reliability           |  🟠 MEDIUM  | ◑ PARTIAL — `failover.recovery.partial.total` metric + `WARN` + documented `merge` policy done (`PayloadGather`); optional all-or-nothing gather mode not added                             |
| I-04 | `RecoveredPayloadHandler` failure isolation (try/catch + ERROR log)                   | Reliability           |  🟠 MEDIUM  | ✅ DONE — `AdvancedFailoverHandler` guards the handler, logs `ERROR`, returns raw payload                                                                                                    |
| I-05 | Virtual-thread `SecurityContext` propagation trap; ship reference `ContextPropagator` | Reliability           |  🟠 MEDIUM  | ◑ PARTIAL — `ContextPropagator` SPI + `Mdc`/`Micrometer`/`Tenant`/`Composite` impls shipped; no dedicated Spring Security propagator                                                        |
| I-06 | `maven-enforcer-plugin` `<fail>false</fail>`                                          | Build                 |  🟠 MEDIUM  | ✅ DONE — parent POM now `<fail>true</fail>`                                                                                                                                                 |
| I-07 | SPI interface Javadoc `@implSpec` contracts missing                                   | Maintainability       |  🟠 MEDIUM  | ✅ DONE — interface-level **Implementation contract** Javadoc (`FailoverStore`, `FailoverHandler`, …) + `ExpiryPolicyContractVerifier` kit (prose contract, not the literal `@implSpec` tag) |
| I-08 | Scatter/gather partial-recovery end-to-end IT missing                                 | Testing               |  🟠 MEDIUM  | ✅ DONE — partial-recovery assertions in `FailoverAppTestIT`                                                                                                                                 |
| I-09 | Circuit-breaker state-transition ITs missing                                          | Testing               |   🟡 LOW    | ✅ DONE — `ResilienceFailoverExecutionTest` covers CLOSED→OPEN→HALF_OPEN with real breaker                                                                                                   |
| I-10 | Dashboard fail-closed / role-gate ITs missing                                         | Testing               |   🟡 LOW    | ✅ DONE — `DashboardAutoConfigurationTest` + `DashboardExposureInterceptorTest` + `FailoverDashboardEndToEndIT`                                                                              |
| I-11 | `ScatterGatherFailoverHandler.recoverAll()` LSP carve-out                             | Design                |   🟡 LOW    | ✅ DONE — documented as an **optional operation** (JDK convention) on `FailoverHandler.recoverAll`                                                                                           |
| I-12 | JDBC `expire_on` index not mandated in shipped DDL                                    | Performance           |   🟡 LOW    | ✅ DONE — `EXPIRE_ON` index in DDL snippets + test schemas                                                                                                                                   |
| I-13 | Optional `EncryptingPayloadEnricher` module would reduce PII footguns                 | Security              |   🟡 LOW    | ✅ DONE — `EncryptingSerializer` (JDBC cipher) wired via `failover.store.jdbc.encryption` + docs                                                                                             |
| I-14 | `allow-insecure=true` should be refused under a prod profile                          | Security              |   🟡 LOW    | ✅ DONE — `DashboardAutoConfiguration` refuses `allow-insecure` under the `prod` profile                                                                                                     |
| I-15 | Stale SCM tag `failover_1.1.0` in parent POM                                          | Build                 |   🟡 LOW    | ✅ DONE — `<tag>HEAD</tag>`                                                                                                                                                                  |
| I-16 | Connection-pool tuning guidance absent                                                | Performance           |   🟡 LOW    | ✅ DONE — "Connection Pool Tuning" section in `docs/modules/store-jdbc.md`                                                                                                                   |
| I-17 | Published SBOM (CycloneDX) per release                                                | Supply-chain          |   🟡 LOW    | ⬜ TODO — no CycloneDX/SBOM in build or release workflow                                                                                                                                     |
| I-18 | `domain` vs `name` attribute confusion for new users                                  | Usability             |   🟡 LOW    | ✅ DONE — covered in `quickstart.md` / `key-generation.md`                                                                                                                                   |
| I-19 | `ReferentialPayload` naming ambiguity                                                 | Usability             | 🟢 TRIVIAL  | ⬜ TODO — backlog (semver-breaking rename)                                                                                                                                                   |
| I-20 | 16+ modules add reactor overhead                                                      | Maintainability       | 🟢 TRIVIAL  | ⬜ TODO — by design (fine-grained optional deps); no change planned                                                                                                                          |

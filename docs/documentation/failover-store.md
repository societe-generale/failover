...
failover:
  store:
    type: jdbc
```
The failover information will be stored in **FAILOVER_STORE** table as per the above configurations.

Make sure you have the below failover store table created in your database!

```sql
-- Table name should be :  %table-prefix%FAILOVER_STORE , default prefix is empty string
...
You can further customize the jdbc store, for more details please see the document [customization.md](customization.md)

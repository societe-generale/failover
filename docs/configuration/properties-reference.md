...
| Property | Type | Default | Description |
|---|---|---|---|
| `failover.scheduler.enabled` | `boolean` | `true` | Enable or disable both schedulers (observable + cleanup). |
| `failover.scheduler.report-cron` | `String` | `"0 0 0 * * *"` | Cron expression for the observable scheduler. Default: daily at midnight. |
...
| `failover.exception-policy` | `FAILOVER_EXCEPTION_POLICY` |
| `failover.store.type` | `FAILOVER_STORE_TYPE` |
| `failover.store.async` | `FAILOVER_STORE_ASYNC` |
| `failover.store.jdbc.table-prefix` | `FAILOVER_STORE_JDBC_TABLE_PREFIX` |
| `failover.scheduler.enabled` | `FAILOVER_SCHEDULER_ENABLED` |
| `failover.scheduler.report-cron` | `FAILOVER_SCHEDULER_REPORT_CRON` |
| `failover.scheduler.cleanup-cron` | `FAILOVER_SCHEDULER_CLEANUP_CRON` |
| `failover.scatter.parallel` | `FAILOVER_SCATTER_PARALLEL` |

...
| `failover.dashboard.security.type`                   | `SecurityType` | `AUTHORITY`                                                      | Authorization strategy. `ROLE` — checks role via `hasRole(security.role)` (RBAC); `AUTHORITY` — checks authority via `hasAuthority(security.authority)` (permission-based). See [Security configuration](../modules/dashboard.md#security-fail-closed). |
